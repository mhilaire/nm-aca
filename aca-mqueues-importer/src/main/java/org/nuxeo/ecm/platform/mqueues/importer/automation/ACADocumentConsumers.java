 ...
 * @since 9.1
 */
@Operation(id = ACADocumentConsumers.ID, category = Constants.CAT_SERVICES, label = "Imports document", since = "9.1",
        description = "Import mqueues document into repository.")
public class ACADocumentConsumers {
    private static final Log log = LogFactory.getLog(ACADocumentConsumers.class);
    public static final String ID = "ACA.runDocumentConsumers";
    ...
    public void run() {
        ...
        try (MQManager<DocumentMessage> manager = getManager();
             DocumentConsumerPool<DocumentMessage> consumers = new DocumentConsumerPool<>(getMQName(), manager,
                     new ACADocumentMessageConsumerFactory(repositoryName, rootFolder),
                     consumerPolicy)) {
            consumers.start().get();
        } catch (Exception e) {
